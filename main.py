...
def process_inputs(audio_filepath, image_filepath, video_filepath):
    ...
    if not video_filepath:
        video_request = (
            "Before I provide a recommendation, please upload a short video of the affected area in good lighting "
            "from multiple angles, including gentle movement to assess swelling and extent."
        )
        doctor_audio = convert_text_to_doctor_audio(video_request)
        return patient_text, video_request, str(Path(doctor_audio))
